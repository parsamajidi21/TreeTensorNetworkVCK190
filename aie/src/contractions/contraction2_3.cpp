...
void contraction23(input_stream_float* __restrict in0, input_stream_float* __restrict in1, output_stream_float* __restrict out) {
    ...
    for(int k = 0; k < 644264; k++)chess_prepare_for_pipelining{
        ...
        for (int v = 0; v < 256; v += 16) {
            aie::vector<float, 16> Wv = aie::load_v<16>(&W23[v]);
            if (l < 8){
                switch (l) {
                case 0: 
        ...
        aie::vector<float, 8>out_23_0(ext_elem(out_a,0),  ext_elem(out_a,1),  ext_elem(out_a,2), ext_elem(out_a,3),
                        ext_elem(out_a,4), ext_elem(out_a,5), ext_elem(out_a,6), ext_elem(out_a,7));
        aie::vector<float, 8>out_23_1(ext_elem(out_a_1,0), ext_elem(out_a_1,1), 0, 0, 0, 0, 0, 0);

        out_23_0 = fpmul(out_23_0, scale_vec);
        out_23_1 = fpmul(out_23_1, scale_vec);
        aie::vector<float, 16>out_23 = concat(out_23_0,out_23_1);
        printf("out_23: %f %f %f %f %f %f %f %f %f %f \n", ext_elem(out_23,0),  ext_elem(out_23,1),  ext_elem(out_23,2), ext_elem(out_23,3),
                        ext_elem(out_23,4), ext_elem(out_23,5), ext_elem(out_23,6), ext_elem(out_23,7),
                        ext_elem(out_23,8), ext_elem(out_23,9));
        writeincr(out,out_23);
    }
}
