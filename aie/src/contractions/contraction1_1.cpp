...
void contraction11(input_stream_int32* __restrict in1, input_stream_int32* __restrict in2, output_stream_int32* __restrict out) {
    ...
    out_a_1 = null_v8acc80();
        ...
        for (int v = 0; v < 1600; v += 16) {
            alignas(64) aie::vector<int32_t, 16> Wv = aie::load_v<16>(&W11[v]);
            if (l < 8){
                switch (l){
                    case 0: 
        ...
        aie::vector<int32_t, 16>out_11(ext_elem(srs(out_a,28),0),  ext_elem(srs(out_a,28),1),  ext_elem(srs(out_a,28),2),ext_elem(srs(out_a,28),3),
