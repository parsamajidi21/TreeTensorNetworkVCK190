...
void contraction21(input_stream_float* __restrict in0, input_stream_float* __restrict in1, output_stream_float* __restrict out) {
    ...
    for(int k = 0; k < 644264; k++)chess_prepare_for_pipelining{
        ...
        for (int v = 0; v < 256; v += 16) {
            aie::vector<float, 16> Wv = aie::load_v<16>(&W21[v]);
            if (l < 8){
                switch (l) {
                case 0: 
        ...
        aie::vector<float, 8>out_21_0(ext_elem(out_a,0),  ext_elem(out_a,1),  ext_elem(out_a,2), ext_elem(out_a,3),
                        ext_elem(out_a,4), ext_elem(out_a,5), ext_elem(out_a,6), ext_elem(out_a,7));
        aie::vector<float, 8>out_21_1(ext_elem(out_a_1,0), ext_elem(out_a_1,1), 0, 0, 0, 0, 0, 0);

        out_21_0 = fpmul(out_21_0, scale_vec);
        out_21_1 = fpmul(out_21_1, scale_vec);
        aie::vector<float, 16>out_21 = concat(out_21_0,out_21_1);
        
        writeincr(out,out_21);
    }
}
