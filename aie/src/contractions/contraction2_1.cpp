...
void contraction21(input_stream_int32* __restrict in1, input_stream_int32* __restrict in2, output_stream_int32* __restrict out) {
    ...
    mul_a_21_0 = null_v8acc80();
    mul_a_21_1 = null_v8acc80();    
        in0_v = readincr_v<4, aie_stream_resource_in::a>(in1);
        in1_v = readincr_v<4, aie_stream_resource_in::b>(in2);
        //Outer Product
        mul_a_21_0 = lmul8(yset_v(0,in0_v)/*xbuff*/, 0/*xstart*/, 0x11110000/*xoffset*/, wset_v(0,in1_v)/*zbuff*/, 0/*zstart*/, 0x32103210/*zoffset*/);
        mul_a_21_1 = lmul8(yset_v(0,in0_v)/*xbuff*/, 0/*xstart*/, 0x33332222/*xoffset*/, wset_v(0,in1_v)/*zbuff*/, 0/*zstart*/, 0x32103210/*zoffset*/);
        mul_v[0] = srs(mul_a_21_0,0); /*Convert accumulator to vector*/
        mul_v[1] = srs(mul_a_21_1,0); /*Convert accumulator to vector*/
        int l=0;
        int m=0;
        for (int v = 0; v < 256; v += 16) {
            alignas(64) aie::vector<int32_t, 16> Wv = aie::load_v<16>(&W21[v]);
            if (l < 8){
                switch (l) {
                    case 0:
        ...
        aie::vector<int32_t, 16>out_21(ext_elem(srs(out_a,28),0),  ext_elem(srs(out_a,28),1),  ext_elem(srs(out_a,28),2),ext_elem(srs(out_a,28),3),
